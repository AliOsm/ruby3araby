...
في الدروس السابقة تعلمنا كيف ننشئ أصنافاً ونستخدم `initialize` والموصّلات. لكن ماذا لو أردنا إنشاء أصناف **متشابهة** تشترك في بعض الخصائص والسلوكيات؟ هل نُكرر الكود في كل صنف؟ **بالطبع لا!** هنا تأتي **الوراثة** (Inheritance).
...
**المشكلة:** كود مُكرر! إذا أردنا تعديل `eat` أو `sleep`، يجب تعديلها في كل صنف.
...
class Developer < Employee
  ...
  def work
    puts "#{@name} يكتب كود بلغة #{@language}"
  end
...
**ملاحظة:** لمشاركة الكود بين أصناف غير متعلقة، نستخدم **الوحدات** (Modules) - سنتعلمها في الدرس القادم.
...
- تريد مشاركة كود بين أصناف متشابهة
...
> **تذكّر:** الوراثة تسمح بإنشاء تسلسل هرمي من الأصناف. الصنف الابن يرث كل شيء من الأب ويمكنه إضافة أو تعديل السلوكيات. استخدم `super` لاستدعاء دالة الأب عند الحاجة. في الدرس القادم سنتعلم **الوحدات والـ Mixins** لمشاركة الكود بين أصناف غير متعلقة!
