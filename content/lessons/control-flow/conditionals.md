...
الجمل الشرطية تسمح للبرنامج بتنفيذ شيفرة مختلف بناءً على شرط معين. مثل الحياة اليومية:
...
```ruby
if الشرط
  # الشيفرة التي سيُنفذ إذا كان الشرط صحيحاً
end
```

- `if` تبدأ الجملة الشرطية
- الشرط يُقيّم إلى `true` أو `false`
- الشيفرة بين `if` و `end` يُنفذ فقط إذا كان الشرط `true`
...
ماذا لو أردنا تنفيذ شيفرة بديل عندما يكون الشرط خاطئاً؟ نستخدم `else`:
