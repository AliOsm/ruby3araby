...
`retry` يعيد تنفيذ الكود من بداية `begin`:
...
|--------|--------|----------|
| `begin` | بداية الكود المحمي | دائماً |
| `rescue` | معالجة الخطأ | عند حدوث خطأ |
| `else` | كود النجاح | عند عدم حدوث خطأ |
| `ensure` | كود التنظيف | دائماً (بعد rescue أو else) |
...
| `ensure` | كود يُنفذ دائماً للتنظيف |
| `else` | كود يُنفذ عند النجاح فقط |
