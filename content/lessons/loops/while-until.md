...
في البرمجة، نحتاج أحياناً لتكرار كود معين عدة مرات. بدلاً من كتابة نفس الكود مراراً وتكراراً، نستخدم **الحلقات التكرارية** (Loops). في هذا الدرس، سنتعلم حلقتي `while` و `until`.

## حلقة while (طالما)

حلقة `while` تُكرر الكود **طالما** الشرط صحيح (true). بمجرد أن يصبح الشرط خاطئاً (false)، تتوقف الحلقة.
...
2. **تنفيذ الكود:** اطبع counter، ثم زِد counter بـ 1
...
# هذا الكود لن يتوقف أبداً!
```

> **ملاحظة:** في المتصفح، سيتوقف الكود تلقائياً بعد 60 ثانية لمنع تجميد الصفحة.
...
حلقة `until` هي **عكس** حلقة `while`. تُكرر الكود **حتى** يصبح الشرط صحيحاً.
...
3. **استخدم += و -=** - لتبسيط كود تحديث المتغيرات
